...
# here is the List specific functions. Be careful here as the append(), insert(), del() and sort() are 
# destructive operation (it modifies the list in place instead of of returning a new list). if you used them with print(), they will return None
# therefore, you need to do it outside the print to make the results show up.
an = [ m for m in range(8) if m > 3];    print();    print("using list comprehension ", an);    an.insert(1, 201); print("an.insert(1,11) is ", an)
...
# with parenthesis being optional in many contexts. For single element, you have to put a comma in, otherwise, you will get a warning
...
# if you don't put comma, at very end, it is just like regular assignment operation
